...
class WallDetectionViewController: UIViewController {
    ...
    var messageButton: ButtonOutlined = .createButton(text: "Click on the choosen wall to \n see your art on it")
...
extension WallDetectionViewController {
    ...
    func nextScene() {
        ...
        if image == nil {
            snapshotView.image = ARView.snapshot()
            snapshotView.alpha = 1
        }
        
        let newScene = ArtPublishingController()
        newScene.artView = snapshotView
